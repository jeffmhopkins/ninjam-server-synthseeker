...
class BufferQueue
{
  public:
    ...
    void Clear()
    {
      ...
      m_emptybufs.Empty();
      intptr_t l=m_samplequeue.Available()/4;
      WDL_HeapBuf **bufs=(WDL_HeapBuf **)m_samplequeue.Get();
      if (bufs) while (l--)
      {
        if ((intptr_t)*bufs != 0 && (intptr_t)*bufs != -1) delete *bufs;
        bufs++;
      }
...
void NJClient::on_new_interval()
{
  ...
  for (u = 0; u < m_locchannels.GetSize() && u < m_max_localch; u ++)
  {
    ...
    intptr_t wasact=lc->bcast_active;
...
{
  m_cs.Enter();
  if (b && (intptr_t)b != -1) m_emptybufs.Add(b);
  m_cs.Leave();
}
...
void BufferQueue::AddBlock(float *samples, int len, float *samples2)
{
  ...
  if (len>0)
  {
    ...
    intptr_t uselen=len*sizeof(float);
    if (samples2)
    {
      uselen+=uselen;
    }
